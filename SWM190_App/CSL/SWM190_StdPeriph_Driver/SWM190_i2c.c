...
void I2C_Init(I2C_TypeDef * I2Cx, I2C_InitStructure * initStruct)
{
	...
	{		
		...
		I2Cx->CLKDIV = (SystemCoreClock/2)/5/initStruct->MstClk;	//I2C的工作时钟是系统时钟二分频
		...
		case ((uint32_t)I2C1):
			if(initStruct->MstIEn)
			{
				NVIC_EnableIRQ(GPIOC5_I2C1_IRQn);
			}
			else
			{
				NVIC_DisableIRQ(GPIOC5_I2C1_IRQn);
			}
			break;
		}
	}
...
{
	I2Cx->MSTCR |= (0x01 << I2C_MSTCR_EN_Pos);
}
