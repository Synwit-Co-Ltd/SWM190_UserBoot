...
{	
	...
	{
	case ((uint32_t)ADC0):		
		if(initStruct->EOC_IEn | initStruct->OVF_IEn) NVIC_EnableIRQ(GPIOC4_ADC0_IRQn);
		break;
	
	case ((uint32_t)ADC1):		
		if(initStruct->EOC_IEn | initStruct->OVF_IEn) NVIC_EnableIRQ(ADC1_GPIOA14_IRQn);
		break;
	}
}
