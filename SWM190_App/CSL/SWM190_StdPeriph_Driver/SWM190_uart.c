...
{	
	...
	case ((uint32_t)UART2):		
		if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn)
		{
			NVIC_EnableIRQ(UART2_GPIOB4_IRQn);
		}
		else
		{
			NVIC_DisableIRQ(UART2_GPIOB4_IRQn);
		}
		break;
	...
	case ((uint32_t)UART3):		
		if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn)
		{
			NVIC_EnableIRQ(UART3_GPIOB5_IRQn);
		}
		else
		{
			NVIC_DisableIRQ(UART3_GPIOB5_IRQn);
		}
		break;
	}
}
