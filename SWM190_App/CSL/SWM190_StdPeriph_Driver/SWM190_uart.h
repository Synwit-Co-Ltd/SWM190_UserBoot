#ifndef __SWM190_UART_H__
#define __SWM190_UART_H__
...
typedef struct {
	...
	uint16_t TimeoutTime;		//超时时长 = TimeoutTime * (10 / Baudrate) 秒
	uint8_t  TimeoutIEn;		//超时中断，超过 TimeoutTime * (10 / Baudrate) 秒没有在RX线上接收到数据时触发中断
} UART_InitStructure;
