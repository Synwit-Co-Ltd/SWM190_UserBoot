...
void SPI_Init(SPI_TypeDef * SPIx, SPI_InitStructure * initStruct)
{
	...
	case ((uint32_t)SPI1):		
		if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TXCompleteIEn)
		{
			NVIC_EnableIRQ(SPI1_BTIMR1_IRQn);
		}
		else
		{
			NVIC_DisableIRQ(SPI1_BTIMR1_IRQn);
		}
		break;
	}
}
...
{
	return (SPIx->IF & SPI_IF_RFOV_Msk) ? 1 : 0;
}
...
/****************************************************************************************************************************************** 
* 函数名称:	SPI_INTRXThresholdEn()
* 功能说明:	接收FIFO中数据个数≥CTRL.RXTHR中断使能
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
{
	SPIx->IE |= (0x01 << SPI_IE_RFTHR_Pos);
}

/****************************************************************************************************************************************** 
* 函数名称:	SPI_INTRXThresholdDis()
* 功能说明:	接收FIFO中数据个数≥CTRL.RXTHR中断禁止
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
/****************************************************************************************************************************************** 
* 函数名称:	SPI_INTRXThresholdClr()
* 功能说明:	接收FIFO中数据个数≥CTRL.RXTHR中断标志清除
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
{
	SPIx->IF = (1 << SPI_IF_RFTHR_Pos);
}

/****************************************************************************************************************************************** 
* 函数名称:	SPI_INTRXThresholdStat()
* 功能说明:	接收FIFO中数据个数≥CTRL.RXTHR中断状态
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1
* 输    出: uint32_t				1 接收FIFO中数据个数≥CTRL.RXTHR    0 接收FIFO中数据个数不≥CTRL.RXTHR
* 注意事项: 无
******************************************************************************************************************************************/
uint32_t SPI_INTRXThresholdStat(SPI_TypeDef * SPIx)
{
	return (SPIx->IF & SPI_IF_RFTHR_Msk) ? 1 : 0;
}

/****************************************************************************************************************************************** 
* 函数名称:	SPI_INTTXThresholdEn()
* 功能说明:	发送FIFO中数据个数≤CTRL.TXTHR中断使能
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
{
	SPIx->IE |= (0x01 << SPI_IE_TFTHR_Pos);
}

/****************************************************************************************************************************************** 
* 函数名称:	SPI_INTTXThresholdDis()
* 功能说明:发送FIFO中数据个数≤CTRL.TXTHR中断禁止
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
/****************************************************************************************************************************************** 
* 函数名称:	SPI_INTTXThresholdClr()
* 功能说明:	发送FIFO中数据个数≤CTRL.TXTHR中断标志清除
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1
* 输    出: 无
* 注意事项: 无
******************************************************************************************************************************************/
...
{
	SPIx->IF = (1 << SPI_IF_TFTHR_Pos);
}

/****************************************************************************************************************************************** 
* 函数名称:	SPI_INTTXThresholdStat()
* 功能说明:	发送FIFO中数据个数≤CTRL.TXTHR中断状态
* 输    入: SPI_TypeDef * SPIx		指定要被设置的SPI，有效值包括SPI0、SPI1
* 输    出: uint32_t				1 发送FIFO中数据个数≤CTRL.TXTHR    0 发送FIFO中数据个数不≤CTRL.TXTHR
* 注意事项: 无
******************************************************************************************************************************************/
uint32_t SPI_INTTXThresholdStat(SPI_TypeDef * SPIx)
{
	return (SPIx->IF & SPI_IF_TFTHR_Msk) ? 1 : 0;
}
